...
export function Home() {
  ...
  const settings = {
    ...
    googleReviewsUrl: tenantSettings?.googleReviewsUrl || DEFAULT_SETTINGS.googleReviewsUrl,
  ...
  return (
    <div className={`flex flex-col ${plan === 'enterprise' ? 'bg-marfil relative before:content-[""] before:absolute before:inset-0 before:pointer-events-none before:opacity-[0.04] before:bg-[url("https://www.transparenttextures.com/patterns/canvas-paper.png")]' : ''}`}>
      ...
      {/* Testimonials Section (Premium & Enterprise) - Integrated with Google Reviews */}
      {(plan === 'premium' || plan === 'enterprise') && (
        <section className="bg-marfil py-24">
          <div className="container mx-auto px-4 md:px-12">
            <div className="mb-12 text-center flex flex-col items-center">
              <span className="mb-3 block text-[10px] font-bold tracking-[0.3em] uppercase text-terracota">Reseñas Verificadas en Google</span>
              <h2 className="font-serif text-4xl font-semibold text-gris md:text-5xl">La voz de nuestra Comunidad</h2>
              <div className="mx-auto mt-4 h-1 w-20 bg-salvia mb-6"></div>
              ...
              <a 
                ...
                className="inline-flex items-center gap-3 bg-white/80 border border-arena/40 px-5 py-2.5 rounded-full shadow-sm hover:shadow-md transition-all hover:scale-105 group cursor-pointer"
              >
                ...
                <div className="flex items-center gap-1.5 text-xs font-bold text-gris">
                  ...
                  <span className="text-gris/50 font-normal ml-1 group-hover:text-salvia transition-colors">en Google Maps ↗</span>
                </div>
              </a>
            </div>
            ...
            <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
              {[
                { name: "Sofía M.", role: "Opinión en Google Reviews", text: "Un espacio único de paz y luz en Quito. Las clases me han ayudado a reconectar con mi centro y encontrar flexibilidad física y mental." },
                { name: "Carlos R.", role: "Opinión en Google Reviews", text: "Excelente nivel de instructores y una plataforma impecable. Me encanta poder reservar mis clases y dar seguimiento a mi progreso." },
                { name: "Elena G.", role: "Opinión en Google Reviews", text: "El ambiente zen se respira en cada rincón. Instructores muy preparados y una atención súper cálida. 100% recomendado." }
              ].map((t, idx) => (
                <motion.div
                  key={idx}
                  initial={{ opacity: 0, scale: 0.95 }}
                  ...
                  className="rounded-[32px] border-[8px] border-white bg-arena/30 p-8 shadow-sm flex flex-col justify-between hover:shadow-md transition-all"
                >
                  <div>
                    <div className="flex items-center justify-between mb-4">
                      <span className="text-amber-500 text-sm">★★★★★</span>
                      <span className="text-[9px] font-bold uppercase tracking-widest text-salvia bg-salvia/10 px-2.5 py-1 rounded-full flex items-center gap-1 border border-salvia/20">
            ...
            {/* CTA Buttons for Google Reviews */}
            <div className="mt-14 flex flex-col sm:flex-row items-center justify-center gap-4">
              <a 
                ...
                rel="noopener noreferrer"
              >
                <Button className="rounded-full bg-gris px-8 py-5 text-xs font-bold uppercase tracking-widest text-white hover:bg-salvia transition-colors shadow-md flex items-center gap-2 cursor-pointer">
              ...
              <a 
                ...
                rel="noopener noreferrer"
              >
                <Button variant="outline" className="rounded-full border border-salvia px-8 py-5 text-xs font-bold uppercase tracking-widest text-salvia hover:bg-salvia/10 transition-colors shadow-sm flex items-center gap-2 cursor-pointer">
                  <span>✍️ Déjanos tu opinión en Google</span>
                </Button>
              </a>
            </div>
          </div>
